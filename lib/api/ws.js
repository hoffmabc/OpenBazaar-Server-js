...
const ALLOWED_TAGS = ['h2', 'h3', 'h4', 'h5', 'h6', 'p', 'a', 'u', 'ul', 'ol', 'nl', 'li', 'b', 'i',
...
class WSProtocol {
    ...
    **/
    ...
    getVendors(messageId, quantity) {
        let queried, vendors, toQuery;

        if(quantity == 0) {
            return
        }
        if(this.factory.outstandingVendors.includes(messageId)) {
            queried = this.factory.outstandingVendors[messageId];
        } else {
            queried = [];
            this.factory.outstandingVendors = [];
            this.factory.outstandingVendors[messageId] = queried;
        }

        vendors = this.factory.mserver.protocol.multiplexer.vendors.values();

        shuffle(vendors);
        toQuery = [];
        for(var vendorIndex in vendors) {
            toQuery.push(vendors[vendorIndex]);
        }

        function handleResponse(metadata, node) {
            let vendor;
            ...
            if(metadata !== undefined) {
                vendor = {
                    ...
                    "vendor": {
                        ...
                        "avatar_hash": metadata.avatarHash.encode("hex"),
                        "nsfw": metadata.nsfw
                    }
                };
                self.transport.write(json.dumps(sanitizeHtml(vendor), 4));
                queried.push(node.id);
                return true;
            } else {
                if(node.id in self.factory.mserver.protocol.multiplexer.vendors) {
                    delete self.factory.mserver.protocol.multiplexer.vendors[node.id];
                }
        ...
        for(node in toQuery.slice(0, quantity)) {
            self.factory.mserver.getUserMetadata(node).addCallback(handleResponse, node);
        }
...
// self.factory.kserver.resolveGUID(unhexlify(guid)).addCallback(send)
//
...
class WSFactory extends Factory {
    constructor(marketServer, kademliaServer, onlyIP) {
        if(onlyIP === undefined) {
            onlyIP = ['127.0.0.1'];
        }
        this.mserver = marketServer;
        this.kserver = kademliaServer;
        this.db = marketServer.db;
        this.outstandingListings = [];
        this.outstandingVendors = [];
    ...
    buildProtocol(addr) {
        if(!this.onlyIP.includes(addr.host) && !this.onlyIP.includes('0.0.0.0')) {
            return;
        }
        return Factory.buildProtocol(this, addr);
    }
    ...
    push(msg) {
        for(var clientIndex in this.clients) {
            this.clients[clientIndex].transport.write(msg);
        }
    }
}
