...
switch (command) {
    ...
    function startServer(keys, firstStartup, db) {
        ...
        let ssdp = natUPNP.ssdp.create();

        let p = ssdp.search('urn:schemas-upnp-org:device:InternetGatewayDevice:1');

        p.on('device', function(device) {
            console.log("Found Gateway!", device);
                ...
                client.portMapping({
                    ...
                    if (!err) {
                        ...
                        checkNat(socket, server, (natType, ip, port) => {
                            ...
                            // let wsAPI = new WSFactory(mserver, kserver, ALLOWIP);
                            // let wsFactory = AuthenticatedWebSocketFactory(wsAPI);
                            // wsFactory.authenticated_sessions = authenticatedSessions;
                            // wsFactory.protocol = AuthenticatedWebSocketProtocol
                            // if(SSL) {
                            //     reactor.listenSSL(WSPORT, ws_factory, ChainedOpenSSLContextFactory(SSL_KEY, SSL_CERT), interface);
                            // } else {
                            //     reactor.listenTCP(WSPORT, ws_factory, interface);
                            // }
