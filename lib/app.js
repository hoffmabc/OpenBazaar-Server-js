  ...
  function connectToNetwork(keys) {
    ...
    checkNat(socket, server, (natType, ip, port) => {
      ...
      if (natType != NATTYPE['FULL_CONE']) {
        ...
        for (let seed in SEED_URLS) {
          console.log('Trying: ', SEED_URLS[seed]);
          hostname = url.parse('http://' + SEED_URLS[seed][0]).hostname;
          ds.push(
            new Promise((resolve, reject) => {
              resolve(dns.lookup(hostname));
            })
          )
        }
      }

      Promise.all(ds).then((relayNodes) => {

        let relayHostIP = relayNodes[0];
