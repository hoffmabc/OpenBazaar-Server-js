...
class VendorStore {
    ...
    **/
    ...
    saveVendor(guid, serializedNode) {
        ...
        sqlite.run(`INSERT OR REPLACE INTO vendors(guid, serializedNode) VALUES ('${guid}', '${serializedNode}')`, function(res) {
            if(res.error) {
                console.log('[Datastore] Error saving vendor to database', res.error);
            }
        });
        sqlite.close();
    }
    ...
    getVendors() {
        ...
        sqlite.connect(this.path);
        rows = sqlite.run(`SELECT serializedNode FROM vendors`);
        sqlite.close();

        for(let i=0, len=rows.length; i<len; i++) {
            try {
                row = rows[i];
                let protoNode = root.get('Node');
                let sNode = Node.create(row);
                let dhtNode = new Node.DHTNode(
                    sNode.guid,
                    sNode.nodeAddress.ip,
                    sNode.nodeAddress.port,
                    sNode.publicKey,
                    (!sNode.hasAttribute('relayAddress')) ? null : [sNode.relayAddress.ip, sNode.relayAddress.port]
                );

                nodes[dhtNode.id] = dhtNode;
            } catch(err) {
                console.log('[Datastore] Error deserializing node');
            }
        }
        return nodes;
    ...
    deleteVendor(guid) {
        sqlite.connect(this.path);
        sqlite.run(`DELETE FROM vendors WHERE guid='${guid}'`, function(res) {
            if(res.error) {
                console.log('[Datastore] Error deleting vendor from database', res.error);
            }
        });
    }
}
