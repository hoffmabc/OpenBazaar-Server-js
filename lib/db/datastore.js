...
class KeyStore {
  ...
  **/
  ...
  setKey(keyType, privkey, pubkey) {
    this.db = new TransactionDatabase(new sqlite3.Database(this.path));

    this.db.beginTransaction(function(err, transaction) {
      //this.db.textFactory = str

        let stmt = transaction.prepare('INSERT OR REPLACE INTO keys(type, privkey, pubkey) VALUES (?,?,?)');
        stmt.run(keyType, privkey, pubkey);
  ...
  getKey(keyType) {
    ...
    this.db.all(`SELECT privkey, pubkey FROM keys WHERE type = '${keyType}'`, function(err, rows) {
      if(!err) {
        console.log(rows[0]);
      } else {
        console.log(err);
