  ...
  refreshTable(server) {
    ...
    **/
    ...
    function republishKeys(_) {
      ...
      var neighbors = server.protocol.router.findNeighbors(server.node, exclude=server.node);
  ...
  querySeed(listSeedPubkey) {
    ...
    **/
    ...
    if(listSeedPubkey === undefined) {
        ...
        for(var sp in listSeedPubkey) {
            let seed = sp[0];
            let key = sp[1];
            try {
                console.log('Querying ', seed, ' for peers');
