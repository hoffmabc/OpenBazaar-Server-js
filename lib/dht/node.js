...
class DHTNode {
    ...
    getProto() {
        ...
        if (this.relayNode !== '') {
            nodeObj.relayAddress = {
                ip: this.relayNode[0],
                port: this.relayNode[1]
            }
        }

        let proto = node.create(nodeObj);

        let result = node.encode(proto).finish();
...
class NodeHeap {
    ...
    getUncontacted() {
        let uncontacted = [];
        for (var n in this.heap) {
            if (this.contacted.includes(this.heap[n].id)) {
                uncontacted.push(this.heap[n]);
            }
        }
        return uncontacted;
    }
