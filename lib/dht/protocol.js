...
module.exports = class KademliaProtocol extends RPCProtocol {
  constructor(sourceNode, storage, ksize, database, signingKey) {
    super(sourceNode, new Routing.RoutingTable(ksize, sourceNode));
    this.ksize = ksize;
  ...
  getRefreshIDs() {
    ...
    //             this.recent_transfers.pop()
    //         this.recent_transfers.add(node.id)
    //         this.log.debug("found a new node, transferring key/values")
    //         reactor.callLater(1, this.transferKeyValues, node)
    //     this.router.addContact(node)
    //
    // def isNewConnection(self, node):
    //     if (node.ip, node.port) in this.multiplexer:
    //         return this.multiplexer[(node.ip, node.port)].handler.check_new_connection()
    //     else:
    //         return False
    //
    // def __iter__(self):
