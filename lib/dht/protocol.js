...
module.exports = class KademliaProtocol extends RPCProtocol {
    constructor(sourceNode, storage, ksize, database, signingKey) {
        ...
        this.handledCommands = ['PING', 'STUN', 'STORE', 'DELETE', 'FIND_NODE', 'FIND_VALUE', 'HOLE_PUNCH', 'INV', 'VALUES'];
    ...
    rpc_ping(that, sender, args) {
        that.addToRouter(sender);
        let proto = that.sourceNode.getProto();
        console.log('Pinging', sender.ip + ':' + sender.port);
        return proto;
    }

    rpc_inv(that, sender, serializedInvs) {
        that.addToRouter(sender);
        let ret = [];
        for(var invIndex in serializedInvs) {
            let inv = serializedInvs[invIndex];
            try {
                let d = objects.Inv.deserializeBinary(inv);
                if(that.storage.getSpecific(d.getKeyword(), d.getValuekey()) === undefined) {
                    ret.push(inv);
                }
            } catch(err) {
                console.log(err);
    ...
    rpc_values(that, sender, serializedValues) {
        that.addToRouter(sender);
        for(var valIndex in serializedValues.slice(0,100)) {
            let val = serializedValues[valIndex];
            try {
                let v = objects.Value.deserializeBinary(val);
                that.storage[v.keyword] = [v.getValuekey(), v.getSerializeddata(), parseInt(v.getTtl())];
            } catch(err) {
                console.log(err);
            }
            return ['True'];
    ...
    callPing(nodeToAsk) {
        this.rpcProxy(
            ...
            this.handleCallResponse(result);
        });
    }

    callFindNode(protocol, nodeToAsk, nodeToFind) {
        return protocol.rpcProxy(
            'find_node',
            nodeToAsk,
            [new Uint8Array(Buffer(nodeToFind.id, 'hex'))]
        ).then((result) => {
            protocol.handleCallResponse(result);
            return result;
        }).catch((err) => {
            console.log('callFindNode error:', err);
        });
    }
    ...
    handleCallResponse(result, node) {
        ...
        **/
        node = node || result[0];
        if(result[0]) {
            if(this.isNewConnection(node) && !this.recentTransfers.has(Buffer(node.id).toString('hex'))) {
                // for(var i=0, len=this.recentTransfers.length; i<len; i++) {
                //     this.recentTransfers.pop();
                // }
                // this.recentTransfers.add(node.id);
                // console.log('[Kademlia Server] Call response from new node, transferring key/values');
                // setTimeout(() => {
                //     //this.transferKeyValues(node)
                // }, 1000);
            }
            this.router.addContact(node);
        } else {
            console.log('[Kademlia Server] No response from', node, ', removing from router...');
    ...
    addToRouter(node) {
        ...
        if(!this.isNewConnection(node) && !this.recentTransfers.has(node.id)) {
