...
class RoutingTable {
    ...
    findNeighbors(node, k, exclude) {
        ...
        for (var i = 0; i < ttSize; i++) {
            next = tt.next();
            if (exclude === undefined || !next.sameHomeAs(exclude)) {
        ...
        if (ttSize > 0) {
            let neighbors = [];
            let nextNeighbor;
            for (let i=0, len=Math.min(ttSize, k); i<len; i++) {
                nextNeighbor = nodes[i];
...
class KBucket {
    ...
    removeNode(node) {
        if (!this.nodes.has(node.id.toString('hex'))) {
            return;
        }

        // Delete node, and see if we can add a replacement
        this.nodes.remove(node.id.toString('hex'));
        if (this.replacementNodes.size > 0) {
            let newnode = this.replacementNodes.next();
            this.nodes.append(newnode.id, newnode);
        }
    }
    ...
    addNode(node) {
         ...
         **/
        let hexID = Buffer(node.id).toString('hex');
        if (this.nodes.has(hexID)) {
            this.nodes.remove(hexID);
            this.nodes.append(hexID, node);
    ...
    split() {
        ...
        this.nodes.forEach((node, key, index) => {
            ...
            bucket.nodes.append(node.id, node);
        });
        return [one, two];
    }
