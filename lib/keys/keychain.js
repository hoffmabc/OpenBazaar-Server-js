...
module.exports = class KeyChain {
    ...
    createKeychain(callback) {
        ...
        **/
        ...
        this.db.keys.setKey("guid", this.signingKey.encode(encoder=nacl.encoding.HexEncoder),
                             this.verifyKey.encode(encoder=nacl.encoding.HexEncoder))

        this.bitcoin_master_privkey = bitcointools.bip32_master_key(bitcointools.sha256(self.signing_key.encode()))
        this.bitcoin_master_pubkey = bitcointools.bip32_privtopub(self.bitcoin_master_privkey)
        this.db.keys.setKey("bitcoin", self.bitcoin_master_privkey, self.bitcoin_master_pubkey)

        this.encryptionKey = self.signingKey.to_curve25519_private_key()
        this.encryptionPubkey = self.verifyKey.to_curve25519_public_key()
