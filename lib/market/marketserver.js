...
const MarketProtocol = require('./protocol.js');
...
module.exports = class MarketServer {
    ...
    **/
    constructor(kserver, signingKey, database, audit) {
        this.audit = audit || true;
        this.kserver = kserver;
        this.signingKey = signingKey;
        this.router = kserver.protocol.router;
        this.db = database;
        this.protocol = new MarketProtocol(kserver.node, this.router, signingKey, database, audit);
        // Looping call every 3600 seconds to update listings and do it now
    }

    querySeed(listSeedPubkey) {
        /**
        ...
        **/
        for(let seedIndex in listSeedPubkey) {
            let seed = listSeedPubkey[seedIndex][0];
            let pubkey = listSeedPubkey[seedIndex][1];
        }
        try {
            console.log('Querying', seed, 'for vendors');
        } catch(err) {
            console.log('Error');
        }
    }

    getProfile(nodeToAsk) {
        /**
        ...
        **/
        function getResult(result) {
            try {
                ...
                //verify result
                let p = objects.Profile.deserializeBinary(result[1][0]);
        ...
        if(nodeToAsk.ip === undefined) {
            return false;
        }
        console.log('[Market Server] Fetching profile from', Buffer(nodeToAsk.id).toString('hex'));
        return new Promise((resolve, reject) => {
            this.protocol.callGetProfile(nodeToAsk);
        }).catch((err) => {
            console.log('Call Get Profile', err);
        }).then((result) => {
            getResult(result);
        }, (err) => {
            console.log(err);
