...
module.exports = class MarketProtocol extends RPCProtocol {
    ...
    callGetProfile(nodeToAsk) {
        return this.rpcProxy(
            ...
            console.log('[Market Protocol] callGetProfile error:', err);
        });
        // d = self.get_profile(nodeToAsk)
        // return d.addCallback(self.handleCallResponse, nodeToAsk)
    }

    rpc_get_profile(sender) {
        console.log('[Market Protocol] Serving profile to', sender);
        this.router.addContact(sender);
        try {
            let proto = Profile(this.db).get(true);
            return [proto, this.signingKey.sign(proto).substring(0,64)];
        } catch(err) {
            console.log('[Market Protocol] Unable to load profile to serve');
            return false;
        }
    }
