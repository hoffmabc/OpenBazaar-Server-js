...
class ConnHandler {
    ...
    receiveMessage(bufferedDatagram) {
        ...
        try {
            ...
            let d = message.Message.deserializeBinary(datagram);

            console.log('[OpenBazaarProtocol]', d.getCommand(),'command sent');

            let mSender = d.getSender();
            let senderNode = mSender.getNodeaddress();
            ...
            if(this.timeLastMessage == 0) {
                ...
                for(var i=0, len = this.processors.length; i<len; i++) {
                    ...
                    if(process.handledCommands.includes(COMMANDS[d.getCommand()]) || d.getCommand() == 'NOT_FOUND') {
                        process.receiveMessage(d, this.node, this.connection, this.banScore);
                    }
                }
