...
module.exports = class RPCProtocol {
  ...
  _createWrappedMessage(commandName, msgID) {
  ...
  rpcProxy(commandName, node, args) {
    ...
    let wrappedData = this._createWrappedMessage(commandNameValue, msgID);
    ...
    if (commandName != COMMAND['HOLE_PUNCH']) {
      ...
      console.log(`Calling remote function ${commandName} on ${address} (msgid ${new Buffer(msgID).toString('base64')})`);
    }
