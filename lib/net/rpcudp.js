...
module.exports = class RPCProtocol {
    ...
    constructor(sourceNode, router, waitTimeout) {
        ...
        this.router = router;
        this._waitTimeout = waitTimeout || 30;
        this._outstanding = {};
    }
    ...
    rpcProxy(name, node, args) {
        ...
        let pMessage = root.get('Message'); // Protobuf Message object

        let m = pMessage.create({
            messageID: msgID,
            protoVer: CONFIG.PROTOCOL_VERSION,
            sender: this.sourceNode.getProto(),
            command: COMMAND[name.toUpperCase()],
            testnet: this.multiplexer.testnet
        });
        ...
        let finishedProto = pMessage.encode(m).finish();

        // Sign the protobuf
        m.signature = sodium.crypto_sign(
            new Buffer(finishedProto.toString().substring(0,64), 'utf8'),
            new Buffer(this.signingKey)
        );

        let data = pMessage.encode(m).finish();
        ...
        if(m.command != COMMAND['HOLE_PUNCH']) {
            //let timeout = reactor.callLater(self._waitTimeout, self.timeout, node);
            //this._outstanding[msgID] = [d, address, timeout];
