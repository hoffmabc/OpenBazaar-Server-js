...
module.exports = class Connection {
    ...
    _doSendPacket(seqnum) {
        ...
        if (schPacket.retries >= Constants.MAX_RETRANSMISSIONS) {
            ...
            this._proto.sendDatagram(schPacket.rudpPacket, [this.relayAddr[0], this.relayAddr[1]]);
    ...
    sendMessage(message) {
        ...
        for (let segmentIndex in segments) {
            this._segmentQueue.push(segments[segmentIndex]);
        }
    ...
    receivePacket(rudpPacket, fromAddr) {
        ...
        **/
        ...
        if(fromAddr != rudpPacket.sourceAddr && fromAddr != this.relayAddr) {
            this.setRelayAddress([fromAddr.address, fromAddr.port]);
        }
      ...
      **/
      ...
      if(fragments === undefined) {
        ...
        for (var i = 0, len = fragments.length; i < len; i++) {
          payload = payload + fragments[i].payload;
        }
        this.handler.receiveMessage(payload);

        if (this._receiveHeap.includes(this._nextDeliveredSeqnum)) {
          this._attemptDisablingLoopingReceive();
        }
      }
    }
