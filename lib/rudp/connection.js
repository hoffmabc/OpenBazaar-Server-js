...
module.exports = class Connection {
    ...
    constructor(proto, handler, ownAddr, destAddr, relayAddr) {
        ...
        this._loopingSend = new Timer(function(timer) {
            that._dequeueOutboundMessage();
        }, 0, {
            repeat: true
        });

        this._loopingReceive = new Timer(function(timer) {
            that._popReceivedPacket();
        }, 0, {
            repeat: true
        });

        this._ackHandle = new Timer(function() {
            that._sendAck();
        }, 1000);
    ...
    _sendAck() {
        ...
        this._scheduleSendOutOfOrder(ackPacket);
    }

    _startLoopingReceive() {
        this._loopingReceive.start();
    }

    _stopLoopingReceive() {
        this._loopingReceive.stop();
    }

      _startLoopingSend() {
        this._loopingSend.start();
      }

      _stopLoopingSend() {
        this._loopingSend.stop();
      }
    ...
    _sendSyn() {
        ...
        let seqNumber = this._getNextSequenceNumber();
        console.log(`Sending SYN packet [${seqNumber}]`);
    ...
    _doSendPacket(seqnum) {
        ...
        if (schPacket.retries >= Constants.MAX_RETRANSMISSIONS) {
            ...
            schPacket.timeout_cb = new Timer(function() {
                that._doSendPacket(seqnum);
            }, schPacket.timeout);
    ...
    _cancelAckTimeout() {
        ...
        if(!this._ackHandle.isStopped()) {
            this._ackHandle.stop();
        }
    ...
    _scheduleSendInOrder(rudpPacket, timeout) {
        ...
        let timeout_cb = new Timer(function() {
            that._doSendPacket(seqnum);
        }, 0);
    ...
    _attemptEnablingLoopingReceive() {
        ...
        if (this._loopingReceive.isStopped()
    ...
    _attemptEnablingLoopingSend() {
         ...
         **/
        if (this._loopingSend.isStopped()
            && this._state == this.State.CONNECTED
    ...
    receivePacket(rudpPacket, fromAddr) {
        ...
        **/
        ...
        if(rudpPacket.sourceAddr != fromAddr || this.relayAddr != fromAddr) {
        ...
        if(rudpPacket.fin) {
            ...
            if(this._state == this.State.CONNECTING) {
                console.log(`Processing incoming SYN packet [${rudpPacket.sequenceNumber}]`);
    ...
    _processSynPacket(rudpPacket) {
        ...
        **/
        if(rudpPacket.ack > 0) {
            //console.log(`Processing ACK packet [${rudpPacket.ack}]`);
    ...
    _resetAckTimeout(timeout) {
        if(!this._ackHandle.isStopped()) {
            this._ackHandle.stop();
            this._ackHandle.setDelay(timeout);
            this._ackHandle.start();
        } else {
            let that = this;
            this._ackHandle = new Timer(function(timer) {
                that._sendAck();
            }, timeout);
            this._ackHandle.start();
        }
    ...
    _popReceivedPacket() {
      ...
      **/
      ...
      if(fragments === undefined) {
        ...
        lastSeqnum = lastSeqnum[0].sequenceNumber.toNumber();
