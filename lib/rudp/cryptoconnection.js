...
function CryptoConnection(proto,
  handler,
  ownAddr,
  destAddr,
  relayAddr,
  privateKey) {
    ...
    **/
    let connection = new Connection(proto, handler, ownAddr, destAddr, relayAddr);
    ...
    console.log('left nonce', this._leftNonceBytes);

}

CryptoConnection.prototype._makeNonceFromNum = function(num) {
  /**
  ...
  **/
  let cryptoboxNonceSize = Math.floor(tweetnacl.box.nonceLength / 2);
  let rightNonceBytes = `${num}:${cryptoboxNonceSize}`;

  '{0:0{1}}'.format(
      num,
      Math.floor(this._cryptoBox.NONCE_SIZE / 2)
  );
  return rightNonceBytes + self._left_nonce_bytes;
}

CryptoConnection.prototype._finalizePacket = function(rudpPacket) {
  /**
  ...
  **/
  if(rudpPacket.syn) {
  ...
  } else if(this._cryptoBox !== undefined) {
      ...
      rudpPacket.payload = this._cryptoBox.encrypt(rudpPacket.payload, this._makeNonceFromNum(rudpPacket.sequenceNumber));
  }
  var c = new Connection();
  return c._finalizePacket(rudpPacket);
}

module.exports = CryptoConnection;
